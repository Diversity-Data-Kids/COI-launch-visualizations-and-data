...
SQL_load <- function(database  = "DDK",
  ...
  if ( file.exists(file_name)==T & overwrite==F & show_metadata_only==F) {
    ...
    if(!table_id %in% table_ids){
      RMariaDB::dbDisconnect(con); rm(con) # disconnect and remove connection
      stop(paste0("table '",  table_id,"' does not exist in database '", database, "'", " -- please use SQL_table_id_list('", database, "') or SQL_tables('", database, "') to list available tables"))
    } else {
      cat("\n  Table ID: ", table_id, "\n\n")
    }
    ...
    if (show_metadata_only==F) {
      ...
      else if ( !is.null(columns) & is.null(filter)) {
        dt <- RMariaDB::dbGetQuery(con, paste0("SELECT ", columns, " FROM ", table_id, ";"))
      }

      # load all columns and filter rows
      else if ( is.null(columns) & !is.null(filter) ) {
        dt <- RMariaDB::dbGetQuery(con, paste0("SELECT * FROM ", table_id, " WHERE ", filter, ";"))
      }
